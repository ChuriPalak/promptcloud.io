...
export default function DashboardPage() {
  ...
  const [csKeys, setCsKeys] = useState({ apiKey: "", secretKey: "" });
  ...
  const handleTopUp = async (method: string) => {
    ...
    try {
      if (method === "upi") {
        ...
        const data = await res.json();
        alert(`Send ${amt} INR equivalent to:\n${data.address}\nNetwork: ${data.network}`);
      }
      fetchData();
    } catch (e) {
      alert("Top-up failed");
    ...
    <div style={{ minHeight: "100vh", background: "#07040f", color: "#e0e0ff", fontFamily: "system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, sans-serif" }}>
      ...
      <div style={{ marginLeft: "240px", minHeight: "100vh", position: "relative", zIndex: 1 }}>
        ...
        <main style={{ padding: "24px 28px" }}>
            ...
            <>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "24px" }}>
                <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "16px", padding: "24px" }}>
                  ...
                  <div style={{ display: "flex", gap: "8px" }}>
                    <button onClick={() => handleTopUp("upi")} style={{ background: "linear-gradient(135deg, #7c3aed, #a855f7)", border: "none", borderRadius: "10px", padding: "10px 18px", fontSize: "12px", fontWeight: 600, color: "#fff", cursor: "pointer" }}>+ Top Up UPI</button>
          ...
          {["networks", "firewall", "marketplace", "subscriptions"].includes(activeSection) && (
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "16px", padding: "48px", textAlign: "center" }}>
              <h3 style={{ fontSize: "17px", fontWeight: 600, color: "#f0f0ff", margin: "0 0 8px" }}>
                {activeSection === "db" && "Managed DB"}
                {activeSection === "networks" && "Networks"}
                {activeSection === "firewall" && "Firewall"}
                {activeSection === "marketplace" && "Marketplace"}
                {activeSection === "subscriptions" && "Subscriptions"}
                {activeSection === "marketplace" && <span style={{ background: "rgba(255,165,0,0.2)", color: "#ffa500", fontSize: "11px", padding: "2px 6px", borderRadius: "4px", marginLeft: "6px" }}>WIP</span>}
              </h3>
              <p style={{ fontSize: "14px", color: "rgba(160,160,192,0.4)" }}>
                {activeSection === "db" && "Managed databases coming soon. Stay tuned."}
                {activeSection === "networks" && "Private networking and VPC configuration coming soon."}
                {activeSection === "firewall" && "Firewall rule management coming soon."}
                {activeSection === "marketplace" && "One-click app deployments coming soon."}
                {activeSection === "subscriptions" && "Manage your active plans and subscriptions."}
              </p>
            </div>
          )}
        </main>
