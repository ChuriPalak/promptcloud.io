...
const destroyVM    = id           => csRequest('destroyVirtualMachine', { id, expunge: 'true' });
...
const deployVM     = p            => csRequest('deployVirtualMachine', { name: p.name, zoneid: p.zoneId, serviceofferingid: p.offeringId, templateid: p.templateId, networkids: p.networkId });
const listVolumes  = ()           => csRequest('listVolumes', { listall: 'true' }).then(d => d?.listvolumesresponse?.volume || []);
const createVolume = p            => csRequest('createVolume', { name: p.name, zoneid: p.zoneId, diskofferingid: p.diskOfferingId, size: p.size });
const attachVolume = (id, vmId)   => csRequest('attachVolume', { id, virtualmachineid: vmId });
const detachVolume = id           => csRequest('detachVolume', { id });
const deleteVolume = id           => csRequest('deleteVolume', { id });
const listDiskOfferings = ()      => csRequest('listDiskOfferings').then(d => d?.listdiskofferingsresponse?.diskoffering || []);
const listNetworks = ()           => csRequest('listNetworks', { listall: 'true' }).then(d => d?.listnetworksresponse?.network || []);

module.exports = { csRequest, listVMs, getVM, startVM, stopVM, destroyVM, listZones, listOfferings, listTemplates, deployVM, listVolumes, createVolume, attachVolume, detachVolume, deleteVolume, listDiskOfferings, listNetworks };
