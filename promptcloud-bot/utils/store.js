...
db.serialize(() => {
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_pending_payments_user ON pending_payments(user_id)`);
  db.run(`CREATE INDEX IF NOT EXISTS idx_pending_payments_status ON pending_payments(status)`);
});
...
async function getPendingPayments(status = 'pending', limit = 50) {
  return dbAll(
    'SELECT * FROM pending_payments WHERE status = ? ORDER BY created_at DESC LIMIT ?',
    [status, limit]
  );
}

async function getPendingPaymentById(id) {
  return dbGet('SELECT * FROM pending_payments WHERE id = ?', [id]);
}

async function verifyPendingPayment(id, adminId) {
  const payment = await getPendingPaymentById(id);
  if (!payment || payment.status !== 'pending') return { ok: false, error: 'Payment not found or already processed' };

  await dbRun(
    'UPDATE pending_payments SET status = ?, verified_at = ?, verified_by = ? WHERE id = ?',
    ['verified', new Date().toISOString(), adminId, id]
  );

  // Credit the user's wallet
  await addWalletBalance(payment.user_id, payment.amount, payment.currency, `UPI payment verified (UTR: ${payment.utr_number || 'N/A'})`);

  return { ok: true, payment };
}

async function rejectPendingPayment(id, adminId, reason = '') {
  await dbRun(
    'UPDATE pending_payments SET status = ?, verified_at = ?, verified_by = ? WHERE id = ?',
    ['rejected', new Date().toISOString(), adminId, id]
  );
  return { ok: true };
}
